...
;; -------------------------------------------------------------
  ...
  (setq set-background-color "#22272e") ;; This will add a background similar to Github-pages
...
;; -------------------------------------------------------------------------
  ...
  ;; -------------- Inferior docs buffer  ------------------------
  ;; When you use go to definition (g d), you can back to your code buffer
  ;; as it will open the documentation on the right side buffer,
  ;; Map 'C-h l' to move to the left window globally.
  (with-eval-after-load 'help
    (define-key help-mode-map (kbd "C-h") 'evil-window-left))
  ;; ----------------- Inferior Rust Results Buffer --------------
  ...
  ;; --------------- Inferior python Results Buffer --------------
  ;; (defun switch-to-inferior-python-and-start-visual ()
  ;;   "Switch to the inferior-python buffer, go to the right window, and start in visual mode."
  ;;   (interactive) ;; Make the function callable with M-x and keybindings
  ;;   (evil-window-right 1) ;; Move to the window to the right
  ;;   (when (derived-mode-p 'inferior-python-mode) ;; Check if the current mode is inferior-python
  ;;     (evil-visual-state))) ;; Start visual mode if it's an inferior-python buffer
  ;; (with-eval-after-load 'evil-maps
  ;;   (define-key evil-normal-state-map (kbd "C-l") 'switch-to-inferior-python-and-start-visual))

  ;; (defun close-inferior-python-buffer-and-switch-to-visual ()
  ;;   "Set up the 'q' key to close the inferior-python buffer and start in visual mode by default."
  ;;   ;; Map 'q' to 'kill-buffer-and-window' in the local key map
...
of a line."
  ...
  ;; It will look for a file .dir-local-l and load the variable you put,
  ;; and it will tell the make how to build your C++ project.
...
(defvar my-lsp-ui-doc-show-toggled nil
  ...
  ;; (defun my/evil-smart-doc-lookup-and-move-right ()
  ;;   "Call `my/evil-smart-doc-lookup` and then move the window right."
  ;;   (interactive)
  ...
  ;;     (progn
  ;;       (message "Documentation not found, using `xref-find-definitions`.")
  ;;       (xref-find-definitions)
  ;;       (evil-window-move-far-right))))

  ;; (with-eval-after-load 'evil-maps
  ;;   (define-key evil-normal-state-map (kbd "g d") 'my/evil-smart-doc-lookup-and-move-right))
...
;; -------------------------------------------------------------
  ...
  (setq yas-snippet-dirs
        '("~/.emacs.d/private/snippets"                 ;; personal snippets
          "~/.emacs.d/elpa/30.0/develop/yasnippet-snippets-20230815.820/snippets/"
          )
        )
...
;; -------------------------------------------------------------
(add-hook 'markdown-mode-hook 'spacemacs/toggle-line-numbers-on)
;; -------------------------------------------------------------
...
;; -------------------------------------------------------------
;;              Playing videos in Spacemacs
;; -------------------------------------------------------------
;; VLC
(defun play-video-with-vlc (video-file)
  "Play a video file with VLC."
  (interactive "FSelect video file: ")
  (shell-command (format "open -a 'VLC.app' '%s'" video-file)))
;; QUICKTIME
(defun play-video-with-quicktime (video-file)
  "Play a video file with QuickTime Player."
  (interactive "FSelect video file: ")
